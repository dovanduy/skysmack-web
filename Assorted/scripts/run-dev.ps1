...
If ($type -eq "f") {
    # Lerna
    cd "$angular"
    start powershell -argument "-noexit", "lerna bootstrap; lerna run build:npm; lerna run --stream --no-sort --concurrency 10 build:npm:watch"

    # frontend runner
    cd "$angular\ng-skysmack"
    start powershell -argument "-noexit", "npm i; npm run build:lib;npm run ss:c1:open"
} 
ElseIf ($type -eq "q") {
    # Lerna
    cd "$angular"
