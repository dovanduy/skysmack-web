...
import { INVOICE_PAYMENTS_REDUX_KEY, INVOICE_PAYMENTS_REDUCER_KEY } from '../../constants';
...
export function invoicePaymentsReducer(state = new InvoicePaymentsState(), action: ReduxAction, prefix: string = INVOICE_PAYMENTS_REDUX_KEY): InvoicePaymentsState {
    state = sharedReducer(state, action, new InvoicePaymentsState(), INVOICE_PAYMENTS_REDUCER_KEY);
